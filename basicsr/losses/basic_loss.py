...
class D2SMLoss(nn.Module):
    ...
    def init_queue_buffer(self, feature_dim):
        # create the queue
        self.register_buffer("data_queue", torch.randn(self.K, feature_dim))
        self.register_buffer("label_queue", torch.randn(self.K, feature_dim))
    ...
    def _dequeue_and_enqueue_data(self, keys):
        if 'data_queue' not in self:
            self.init_queue_buffer(sum(keys.shape))
    ...
    def forward(self, x, gt):
        ...
        if self.perceptual_weight > 0:
            for k in x_features.keys():
                ...
                with torch.no_grad():
                    self._dequeue_and_enqueue_label(gt_features[k])
